...
fun print_result(sotiris_times,T_grammi,T_stili,parent_array)=
let 
...
fun stayhome(infile:string)=
  let 
      val map_string=linelist(infile)
      ...
      val _=flood_fill_virus((W_grammi,W_stili,0),Q,map,map_grammes-1,map_stiles-1,virus_times,airport_list)
      ...
      val _=flood_fill_sotiris((S_grammi,S_stili,0),Q,map,map_grammes-1,map_stiles-1,sotiris_times,virus_times,parent_array)
  in 
      print_result(sotiris_times,T_grammi,T_stili,parent_array)
  end;

stayhome("stayhome.in17.txt");
